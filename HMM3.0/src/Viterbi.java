...
public class Viterbi extends HMM{
	...
	//delta delta 输出中间结果，局部最大概率；psi 输出中间结果，反向指针指示最可能路径；返回可能性最大的隐藏状态序列的概率
    ...
    {   
        ...
        for (int t = 1; t < ob.length; t++)
        {
            for (int j = 0; j < N; j++)
            {
            	double MaxValue = delta[t - 1][0] * A[0][j];//初始，设第0各状态处的值为最大值
        ...
        for (int i = 1; i < N; i++)
        {
            if (delta[ob.length - 1][i] > probability)
            {
                ...
                q[ob.length - 1] = i;//最后一个时间点的最优状态״̬
            }
        }
